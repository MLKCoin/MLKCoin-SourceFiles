...
uint256 hashGenesisBlock("0x0b5b6da33137005ad7a34ce04c0eb9ea5538c45f6714cfca7672e61689ccbed9");
...
int64 CTransaction::nMinTxFee = 0.00002 * COIN;
/** Fees smaller than this (in satoshi) are considered zero fee (for relaying) */
int64 CTransaction::nMinRelayTxFee = 0.00002 * COIN;
...
const string strMessageMagic = "MLKCoin Signed Message:\n";
...
{
    // MLKCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // MLKCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1397750861;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2535978;
	    ...
	    assert(block.hashMerkleRoot == uint256("0x20b4a9b7ad617dfd7c9add6f0f0e8c6af76ff387520bdc7d0cbcb9e49661dfd9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd4, 0xc4, 0xa4, 0xe3 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MLKCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MLKCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MLKCoinMiner(CWallet *pwallet)
{
    printf("MLKCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("MLKCoin-miner");
    ...
    try { loop {
        ...
        printf("Running MLKCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MLKCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MLKCoinMiner, pwallet));
}
