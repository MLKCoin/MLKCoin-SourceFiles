...
// Copyright (c) 2011-2012 MLKCoin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #MLKCoinTEST0\r");
            Send(hSocket, "WHO #MLKCoinTEST0\r");
        } else {
            // randomly join #MLKCoin00-#MLKCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #MLKCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #MLKCoin%02d\r", channel_number).c_str());
        }
